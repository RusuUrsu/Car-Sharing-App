...
bool CustomerController::deleteCustomer(const std::string& email) {
    ...
    if (!customer.getGdprDeleted() && customer.getId() != "0") { // Check if customer exists and has no orders
...
std::vector<Customer> CustomerController::getAllCustomersSortedByName() {
    auto customers = customerRepo.getCustomers().get<std::vector<json>>();
    std::vector<Customer> sortedCustomers;
    for (auto& cust : customers) {
        sortedCustomers.emplace_back(cust);
    }
